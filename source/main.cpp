...
int main() {
    ...
    array_push(&my_ints, 10);
    array_push(&my_ints, 20);
    array_push(&my_ints, 30);

    printf("Array has %zu elements, capacity for %zu\n", my_ints.length, my_ints.capacity);
    printf("Element at index 1: %d\n", my_ints[1]);

    printf("Iterating with range-based for loop:\n");
    for (int val : my_ints) {
        printf("  - %d\n", val);
    }
    ...
    printf("Array now has %zu elements\n", my_ints.length);

    array_free(&my_ints);
    printf("Array freed. Length is now %zu\n", my_ints.length);

    printf("\nCreating an array with a macro:\n");
    Array<float> my_floats = array_of(1.1f, 2.2f, 3.3f, 4.4f);
